...
class PHPMailer{
	...
	public static function ValidateAddress($address){
		if(defined('PCRE_VERSION')){ //Check this instead of extension_loaded so it works when that function is disabled
			...
			return (strlen($address) >= 3 and
					strpos($address, '@') >= 1 and strpos($address, '@') != strlen($address) - 1);
		}
	}
	...
	public function PreSend(){
		try{
			...
			if(!empty($this->DKIM_domain) && !empty($this->DKIM_private) && !empty($this->DKIM_selector) &&
			   !empty($this->DKIM_domain) && file_exists($this->DKIM_private)
			){
				$header_dkim      = $this->DKIM_Add($this->MIMEHeader .
													$this->mailHeader, $this->EncodeHeader($this->SecureHeader($this->Subject)), $this->MIMEBody);
	...
	function SetLanguage($langcode = 'en', $lang_path = 'language/'){
		...
		$PHPMAILER_LANG['invalid_address']      = '无效的电子邮件地址： ';
		...
		if(isset($images[2])){
			foreach($images[2] as $i => $url){
				...
				if(!preg_match('#^[A-z]+://#', $url)){
					...
					if($this->AddEmbeddedImage($basedir . $directory .
